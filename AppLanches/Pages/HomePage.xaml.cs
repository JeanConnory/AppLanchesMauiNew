...
public partial class HomePage : ContentPage
{
    ...
    private async Task<IEnumerable<Categoria>> GetListaCategorias()
    {
        ...
        {
            await DisplayAlert("Erro", $"Ocorreu um erro inexperado: {ex.Message}", "OK");
            return Enumerable.Empty<Categoria>();
        }
    }
    ...
    private void CvMaisVendidos_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        {
            NavigatedToProdutoDetalhesPage(collectionView, e);
        }
    }
    ...
    private void NavigatedToProdutoDetalhesPage(CollectionView collectionView, SelectionChangedEventArgs e)
    {
        var currentSelection = e.CurrentSelection.FirstOrDefault() as Produto;

        if (currentSelection is null) return;

        Navigation.PushAsync(new ProdutoDetalhePage(currentSelection.Id, currentSelection.Nome!, _apiService, _validator));
